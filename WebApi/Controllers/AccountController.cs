...
using Business.WebApi.Account;
using Business.WebApi.Mailing;
using Common;
...
using Common.UIElements.DataGrid.SourceItems;
using DTO;
...
namespace WebApi.Controllers
{
    ...
    /// </summary>
    [SporComplexHeaderAuthorization]  // needs api key  and channelType
    [Authorize]
    [ExceptionHandler]
    [RoutePrefix("")]
    public class AccountController : ApiController
    {
        ...
        public IHttpActionResult Register(RegisterBindingModel model)
        {
            ...
            if (channelType != ChannelType.WebAdmin)
            {
                if (string.IsNullOrEmpty(model.Email))
                {
                    return BadRequest(ErrorMessage.UserEmailNotExists);
                }

                //comment out after iOs review
                //if (string.IsNullOrEmpty(model.PhoneNumber))
                //{
                //    return BadRequest(ErrorMessage.PhoneNumberNotExists);
                //}
            }
